...
test_SVM_radial <- cbind(test_processed, pred_test_svm_radial[, "Yes", drop = FALSE]) 
...
caret::confusionMatrix(test_SVM_radial$pred_class, 
                       test_SVM_radial$income, 
                       mode="everything",
                       positive='Yes')

# extract ROC
# actual classes first then predicted probabilities
roc_SVM_radial_test <- roc(test_SVM_radial$income, test_SVM_radial$Yes)
...
train_SVM_poly <- cbind(train_processed, pred_train_svm_poly[, "Yes", drop = FALSE]) 
...
caret::confusionMatrix(train_SVM_poly$pred_class, 
                       train_SVM_poly$income, 
                       mode="everything",
                       positive='Yes')

# extract ROC
# actual classes first then predicted probabilities
roc_SVM_poly_train <- roc(train_SVM_poly$income, train_SVM_poly$Yes)
...
test_SVM_poly <- cbind(test_processed, pred_test_svm_poly[, "Yes", drop = FALSE]) 

# test (note the predicted probabilities are now in a column named "Yes")
test_SVM_poly$pred_class <- factor(ifelse(test_SVM_poly$Yes > threshold,"Yes","No" ))

caret::confusionMatrix(test_SVM_poly$pred_class, 
                       test_SVM_poly$income, 
                       mode="everything",
                       positive='Yes')

# extract ROC
# actual classes first then predicted probabilities
roc_SVM_poly_test <- roc(test_SVM_poly$income, test_SVM_poly$Yes)
...
train_NN_pred <- cbind(train_processed, preds_NN_train[, "Yes", drop = FALSE]) # drop = FALSE keeps the original name of this column (yes) in the resulting data frame
test_NN_pred <- cbind(test_processed, preds_NN_test[, "Yes", drop = FALSE])
...
train_NN_pred$pred_class <- factor(ifelse(train_NN_pred$Yes > threshold,"Yes","No"))

# predicted classes first then actual classes
caret::confusionMatrix(
  train_NN_pred$pred_class,
  train_NN_pred$income,
  positive = "Yes",
  mode = "everything"
)

# actual classes first then predicted probabilities
roc_NN_train <- roc(train_NN_pred$income, train_NN_pred$Yes)
auc(roc_NN_train)
plot(roc_NN_train)

# test
test_NN_pred$pred_class <- factor(ifelse(test_NN_pred$Yes > threshold, "Yes", "No"))
...
# actual classes first then predicted probabilities
roc_NN_test <- roc(test_NN_pred$income, test_NN_pred$Yes)
auc(roc_NN_test)
plot(roc_NN_test)
